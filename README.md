...
- **An agent hallucinated a scene architecture that did not exist in the codebase** (a `worldScene.ts` / `sceneState` that were never there). It was caught by diffing the claim against the real file tree, and its output was discarded — which is exactly why it leaves no commit. The takeaway that shaped everything after: agent output is verified against real code, never trusted blindly.
